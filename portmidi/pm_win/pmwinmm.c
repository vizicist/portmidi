...
static void CALLBACK winmm_in_callback(HMIDIIN hMidiIn,
                                       ...
                                       DWORD_PTR dwParam1,
                                       DWORD_PTR dwParam2);
static void CALLBACK winmm_streamout_callback(HMIDIOUT hmo,
	                                          UINT wMsg,
                                              DWORD_PTR dwInstance,
	                                          DWORD_PTR dwParam1, 
                                              DWORD_PTR dwParam2);
...
static void pm_winmm_mapper_input()
{
    ...
    if (wRtn == MMSYSERR_NOERROR) {
        pm_add_device("MMSystem", midi_in_mapper_caps.szPname, TRUE,
                      (UINT) MIDIMAPPER, &pm_winmm_in_dictionary);
    }
}
    ...
    for (i = 0; i < midi_num_outputs; i++) {
        ...
        if (wRtn == MMSYSERR_NOERROR) {
            pm_add_device("MMSystem", midi_out_caps[i].szPname, FALSE,
                          (UINT) i, &pm_winmm_out_dictionary);
        }
    }
}
...
static void pm_winmm_mapper_output()
{
    ...
    if (wRtn == MMSYSERR_NOERROR) {
        pm_add_device("MMSystem", midi_out_mapper_caps.szPname, FALSE,
                      (UINT) MIDIMAPPER, &pm_winmm_out_dictionary);
    }
}
    ...
    DWORD_PTR dwParam2)    /* device timestamp (wrt most recent midiInStart) */
{
    ...
    case MIM_DATA: {
		...
		// This used to be done outside the switch, but some wMsg values
		// don't have the dwInstance set (or it's too early)
		midiwinmm_type m = (midiwinmm_type) midi->descriptor;
...
{
	if (wMsg == MOM_OPEN || wMsg == MOM_CLOSE) {
		return;
	}
	PmInternal *midi = (PmInternal *)dwInstance;
	midiwinmm_type m = (midiwinmm_type) midi->descriptor;
